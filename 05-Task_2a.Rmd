...
In order to use Poisson regression, we assume that the score of the home team is independent of the score of the away team. In order to examine this assumption, we create a table containing the frequency of each combination of scores, with the away-scores horizontally and the home-scores vertically. 

```{r}
scoresTable = table(tippeliga$ya,tippeliga$yh)
scoresTable
```
...
```{r}
chisq.test(scoresTable)
```
This gives a p-value of 0.8873, so there is no evidence aganist our assumption of independence. 
