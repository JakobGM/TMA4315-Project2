## d) Simulation 
In order to investigate the variance in the expected results and find out if the winner really was the best team we simulate 1000 seasons of tippeligaen based on the estimated parameters in point c). The results are displayed in a histogram, sorted based on their position in Tippeligaen 2016. 
```{r}
#Simulation
...
```{r}
sum(rankings["Rosenborg",]==1)/1000
```
This means that with the given strengths Rosenborg would win 89.5 % of the seasons, while Brann would win 5 % and Odd 1.6 %. Thus, we can safely conclude that Rosenborg is truly the best team. Start has a loss rate of 84.1 % and is clearly the weakest team. 
